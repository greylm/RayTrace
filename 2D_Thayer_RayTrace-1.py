...
def get_RayTrace2D_Thayer_global(e_outgoing, stat_height, h_lev_all, n, nr_h_lev_all):
...
def n_gen(pres, hght, tmps, dwpt, stat_height, h_lev_all, h_diff, start_lev): 
    ...
    for i in dwpt:
        if i >= 0:
            pv_i = 6.11*np.exp(17.27*i/(237.3+i)) # in [mb]
        else:
...
def main():
    ...
    # radiosonde data, footer length is file dependent
    pres = np.genfromtxt("radiosonde.txt", skip_header = 8, usecols = 0, skip_footer = 43)
    hght = np.genfromtxt("radiosonde.txt", skip_header = 8, usecols = 1, skip_footer = 43)
    tmps = np.genfromtxt("radiosonde.txt", skip_header = 8, usecols = 2, skip_footer = 43)
    dwpt = np.genfromtxt("radiosonde.txt", skip_header = 8, usecols = 3, skip_footer = 43)
    ...
    get_RayTrace2D_Thayer_global(e_outgoing, stat_height, h_lev_all, n, nr_h_lev_all)
